...
registerBlockType( metadata.name, {
	edit: ( { attributes, setAttributes, clientId } ) => {
		...
		return (
			<>
				<InspectorControls>
					<PanelBody title={ __( 'AI Settings', 'extrachill-content-blocks' ) }>
						<TextareaControl
							label={ __( 'Game Master Persona', 'extrachill-content-blocks' ) }
							value={ gameMasterPersona }
							onChange={ ( value ) => setAttributes( { gameMasterPersona: value } ) }
							help={ __( 'Define the personality and role of the AI storyteller.', 'extrachill-content-blocks' ) }
						/>
					</PanelBody>
				</InspectorControls>
				<div { ...blockProps }>
					<RichText
						...
						value={ title }
						placeholder={ __( 'Enter Adventure Title', 'extrachill-content-blocks' ) }
					/>
					<RichText
						tagName="p"
